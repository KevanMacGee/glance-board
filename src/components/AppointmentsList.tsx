  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      const filteredEvents = mockEvents
        .filter(event => 
          isAfter(event.start, addDays(now, -1)) && 
          isBefore(event.start, fourteenDaysFromNow)
        )
      ...
      <div className="px-6 pb-4 flex-1 overflow-auto gb-scroll" role="list" aria-label="Appointment list">
        <div className="flex flex-col gap-4">
          {events.map((event) => (
            <article key={event.id} className="gb-event" role="listitem">
              <div className="gb-event-time">
                <div className="gb-event-time-value">
                  {formatEventTime(event.start, event.isAllDay)}
                </div>
                <div className="gb-event-time-label">
          ...
          {events.length === 0 && !loading && (
            <div className="text-center py-12 text-gb-faint">
